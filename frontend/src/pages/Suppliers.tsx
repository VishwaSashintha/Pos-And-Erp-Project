...
import { Plus, Search, Edit3, Trash2, Mail, Phone, MapPin, Truck } from 'lucide-react';
...
export const Suppliers: React.FC = () => {
  ...
  const fetchSuppliers = async () => {
    if (!user) return;
    try {
      const data = await apiFetch(`/api/suppliers/${user.tenantId}`);
      setSuppliers(data || []);
    } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !contactName || !phone || !email) return;

    const body = { name, contactName, phone, email, address, taxNumber };

    try {
      if (editingId) {
        await apiFetch(`/api/suppliers/${user?.tenantId}/${editingId}`, {
          method: 'PUT',
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiFetch(`/api/suppliers/${user?.tenantId}/${id}`, {
        method: 'DELETE',
      });
      fetchSuppliers();
    } catch (e) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Suppliers Registry</h1>
          <p className="text-slate-400 text-sm mt-1">Manage vendor contacts and tax profiles.</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center text-slate-500 shadow-xl">
          <Truck size={40} className="mx-auto text-slate-600 mb-4" />
          <p className="font-semibold text-lg text-slate-400 mb-1">No suppliers registered</p>
          <p className="text-sm">Add suppliers to issue Purchase Orders and restock inventory.</p>
        </div>
      ) : (
                  ...
                  <tr key={s.id} className="text-slate-300 hover:bg-slate-850/30 transition-colors">
                    <td className="p-4 pl-6">
                      ...
                      <div className="flex items-center gap-1.5 text-slate-500 text-xs mt-0.5">
                        <MapPin size={12} />
                        <span>{s.address || 'No address registered'}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="text-slate-200 font-semibold">{s.contactName}</div>
                    ...
                    <td className="p-4 font-mono text-xs text-slate-400">{s.taxNumber || 'N/A'}</td>
      ...
      <Modal
        ...
        title={editingId ? 'Edit Supplier Details' : 'Register New Vendor'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">
              Supplier / Company Name <span className="text-indigo-500">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">
                Contact Representative <span className="text-indigo-500">*</span>
              </label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="email"
                required
                placeholder="e.g. sales@bosch.com"
                value={email}
          ...
          <div>
            ...
            <textarea
              placeholder="e.g. 100 Main Street, Warehouse B"
              value={address}
              onChange={(e) => setAddress(e.target.value)}
              rows={3}
              className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-650 focus:outline-none focus:border-indigo-500 text-sm resize-none"
            />
          </div>
